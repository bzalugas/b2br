wall $'\t#Architecture:' `uname -a` \
     ...
     $'\n\t#Last boot:' `last reboot -n1 --time-format iso | awk '/reboot/ {$0=$5;split($0,arr,"T");date=arr[1];split(arr[2],time,":");printf "%s %d:%d", date, time[1], time[2]}'` #uptime -s does the same thing\
